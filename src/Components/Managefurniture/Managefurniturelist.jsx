...
const Managefurniturelist = () => {
  ...
  const deleteproduct = (furnitureID) => {
    fetch(
      `https://ayt-furniture-server.vercel.app/delete-product/${furnitureID}`,
      {
        method: "DELETE",
      }
    )
      ...
      .then((data) => {
        if (data.deletedCount > 0) {
          const filterproduct = datas.filter(
            (data) => data?._id != furnitureID
          );
  ...
  return (
    <>
      <div className="overflow-x-auto">
        ...
        <h2 className="text-2xl font-bold mb-10">MANAGE YOUR PRODUCT HERE</h2>
                ...
                <td className="flex gap-4">
                  <Link to={`/single-product/${furniture._id}`}>
                    <button className="btn btn-outline">
                      <CgDetailsMore className="w-[70%] h-[70%]" />
                    </button>
                  </Link>
                  <button
                    className="btn btn-outline"
                    onClick={() => deleteproduct(furniture?._id)}
                  >
                    <RiDeleteBin6Line className="w-[70%] h-[70%]" />
                  </button>
                  <Link to={`/update-product/${furniture._id}`}>
                    <button className="btn btn-outline">
                      <RxUpdate className="w-[70%] h-[70%]" />
                    </button>
                  </Link>
                </td>
              </tr>
